# Axinite Coverage Plan: 63.3% to 95%

> Generated 2025-03-06 from
> [Codecov](https://app.codecov.io/gh/nearai/ironclaw/tree/main/src)
...
Pure logic, serialization, and database queries testable in isolation without
real infrastructure. Highest coverage gain per unit of effort.

### `src/history/store.rs` -- 0% -> 95% (+1,411 lines)

PostgreSQL repository layer (conversations, jobs, actions, LLM calls,
estimation snapshots). Test query construction and result mapping. Can use the
libSQL backend as a real in-memory database or test doubles for the `Database`
trait.
...
- `test_extension_install_from_manifest` -- parse manifest, create extension
  record
...
- `test_extension_capability_check` -- verify required capabilities before
  activation
...
End-to-end tests that exercise the agent loop, worker, scheduler, and
dispatcher by replaying LLM traces through `TestRig` (see
`tests/support/test_rig.rs`). Each trace test covers multiple modules
simultaneously, making them high-leverage.
...
**Fixture:** `thread_operations.json` **Tests:**
...
**Fixture:** `agent_commands.json` **Tests:**
...
**Fixture:** `worker_multi_turn.json` **Tests:**
...
**Fixture:** `scheduler_parallel.json` **Tests:**
...
**Fixture:** `dispatcher_skills.json` **Tests:**
...
**Covers:** `agent/routine_engine.rs` (~80 lines), `agent/routine.rs` (~40
lines)

Test cron tick and event-triggered routine execution.

**Fixture:** `routine_execution.json` **Tests:**
...
**Covers:** `agent/compaction.rs` (~50 lines), `agent/context_monitor.rs` (~30
lines)

Test turn summarization and memory pressure detection.

**Fixture:** `compaction_flow.json` **Tests:**

- `test_compaction_triggers_at_threshold` -- summarize when context exceeds
  limit
...
**Covers:** `tools/builtin/job.rs` (+308 lines), `tools/builtin/skill_tools.rs`
(+110 lines)
...
**Fixture:** `memory_tools.json` **Tests:**
...
**Fixture:** `extension_management.json` **Tests:**
...
**Fixture:** `self_repair.json` **Tests:**
...
Hardest to test: interactive wizards, Docker, process spawning. Strategy:
extract pure logic into testable functions, test the interactive parts by
injecting mock input.
...
- All trace tests require `--features libsql` and use `TestRigBuilder` from
  `tests/support/`
...
- Setup wizard tests require extracting pure logic from interactive prompts
  first
