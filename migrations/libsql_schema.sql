-- Intentionally kept as one bootstrap schema snapshot so the embedded libSQL
-- initialization path can load the full schema and seed data atomically via a
-- single include. This exceeds the usual 400-line guideline by design.
-- ==================== Migration tracking ====================
...
CREATE TABLE IF NOT EXISTS tool_capabilities (
    ...
    http_allowlist TEXT NOT NULL DEFAULT '[]' CHECK (json_valid(http_allowlist)),
    allowed_secrets TEXT NOT NULL DEFAULT '[]' CHECK (json_valid(allowed_secrets)),
    tool_aliases TEXT NOT NULL DEFAULT '{}' CHECK (json_valid(tool_aliases)),
    ...
    workspace_read_prefixes TEXT NOT NULL DEFAULT '[]' CHECK (json_valid(workspace_read_prefixes)),
