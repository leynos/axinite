...
/// Poison error surfaced when the recorded-writes lock has been poisoned.
type WritesLockPoisoned<'a> = std::sync::PoisonError<std::sync::MutexGuard<'a, Vec<String>>>;
...
impl RecordingSettingsStore {
    ...
    /// Snapshot the recorded setting keys.
    ///
    /// A poisoned lock is propagated rather than panicked on, so the calling
    /// test body decides the verdict.
    fn writes(&self) -> Result<Vec<String>, WritesLockPoisoned<'_>> {
        Ok(self.writes.lock()?.clone())
    }
}
...
impl crate::db::SettingsStore for RecordingSettingsStore {
    ...
    fn set_setting<'a>(
        ...
        Box::pin(async move {
            // The trait signature is fixed, but it is fallible, so a poisoned
            // lock is reported as a database error rather than a panic.
            let mut writes = self.writes.lock().map_err(|_| {
                crate::error::DatabaseError::Query("settings writes lock poisoned".to_string())
            })?;
