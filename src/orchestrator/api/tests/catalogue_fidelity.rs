...
#[rstest]
#[tokio::test]
async fn remote_tool_catalogue_preserves_full_tool_definition_payload(
...
#[tokio::test]
async fn remote_tool_catalogue_version_is_deterministic_and_sensitive_to_content() {
    ...
    registry_c
        .register(build_tool_fixture(
            ToolFixture::CatalogueAlphaWithDifferentPayload,
        ))
        .await;

    let (_tools_a, _instructions_a, version_a) = hosted_remote_tool_catalogue(&registry_a).await;
    let (_tools_b, _instructions_b, version_b) = hosted_remote_tool_catalogue(&registry_b).await;
    let (_tools_c, _instructions_c, version_c) = hosted_remote_tool_catalogue(&registry_c).await;

    assert_eq!(
        version_a, version_b,
        "identical tool sets must produce identical catalogue versions"
    );
    assert_ne!(
        version_a, version_c,
        "different catalogue payloads must produce different catalogue versions"
    );
}

#[tokio::test]
async fn remote_tool_catalogue_version_independent_of_registration_order() {
    let registry_a = Arc::new(ToolRegistry::new());
    registry_a
        .register(build_tool_fixture(ToolFixture::CatalogueAlpha))
        .await;
    ...
    assert_eq!(
        version_a, version_b,
        "catalogue version must be independent of tool registration order"
    );
}
