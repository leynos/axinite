...
fn assert_iteration_bounds(max_iter: usize) {
    ...
    assert!(
        !hit_ceiling,
        "hard_ceiling became reachable before force_text for max_iter={max_iter}"
    );
}
...
proptest! {
    #[test]
    fn iteration_bounds_guarantee_termination(max_iter in 1_usize..129_usize) {
...
async fn test_dispatcher_terminates_with_all_tool_calls_failing() {
    ...
    match inner {
        ...
        Err(crate::error::Error::Llm(crate::error::LlmError::InvalidResponse {
            provider,
            reason,
        })) => {
            assert_eq!(provider, "agent");
            assert!(
                reason.contains("Exceeded maximum tool iterations"),
                "Expected hard-ceiling error reason, got: {reason}"
            );
        }
        Err(other) => {
            panic!("Expected force-text response or hard-ceiling error, got: {other:?}");
        }
    }
}
