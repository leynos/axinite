...
mod tests {
    ...
    struct NotificationHarness {
        notification_rx: mpsc::Receiver<super::RepairNotification>,
    ...
    async fn repair_task_sends_notification_for_stuck_job_success() {
        ...
        let notification =
            tokio::time::timeout(Duration::from_secs(1), harness.notification_rx.recv())
                .await
    ...
    async fn repair_task_deduplicates_manual_required_notifications_for_stuck_jobs() {
        ...
        assert!(
            harness.notification_rx.try_recv().is_err(),
