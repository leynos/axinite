...
/// An open code fence: the fence character (`` ` `` or `~`) and the length of
/// its opening run. A closing fence must repeat the same character at least
/// `len` times.
#[derive(Debug, Clone, Copy)]
struct Fence {
    ch: u8,
...
/// of 3+ identical backticks or tildes. Returns the fence and the index just
/// past the run.
fn parse_opening_fence(bytes: &[u8], line_start: usize) -> Option<(Fence, usize)> {
    // Skip optional leading whitespace
    let run_start = skip_spaces_and_tabs(bytes, line_start);
    if run_start >= bytes.len() || !is_fence_char(bytes[run_start]) {
        return None;
    }
    let ch = bytes[run_start];
    let run_end = fence_run_end(bytes, run_start, ch);
    let len = run_end - run_start;
    if len < 3 {
        return None;
    }
    Some((Fence { ch, len }, run_end))
}
...
fn find_fenced_regions(text: &str) -> Vec<CodeRegion> {
    ...
    while i < bytes.len() {
        ...
        let Some((fence, run_end)) = opening else {
        ...
        // Find closing fence (a line starting with >= fence.len of the same
        // char); an unclosed fence extends to EOF.
        let end = find_closing_fence(text, next, fence).unwrap_or(bytes.len());
        regions.push(CodeRegion {
            start: line_start,
            end,
        });
...
/// fence is unclosed.
fn find_closing_fence(text: &str, from: usize, fence: Fence) -> Option<usize> {
    let bytes = text.as_bytes();
    let mut line_start = from;
    while line_start < bytes.len() {
        if let Some(end) = closing_fence_line_end(bytes, line_start, fence) {
            return Some(end);
        }
...
/// optional indentation, at least `fence.len` repeats of `fence.ch`, then
...
/// not a closing fence.
fn closing_fence_line_end(bytes: &[u8], line_start: usize, fence: Fence) -> Option<usize> {
    // Skip optional leading whitespace
    let run_start = skip_spaces_and_tabs(bytes, line_start);
    let run_end = fence_run_end(bytes, run_start, fence.ch);
    // Must be at least as long as the opening fence
    if run_end - run_start < fence.len {
        return None;
    }
