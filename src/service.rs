//! OS service management for running Axinite as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.axinite.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/axinite.service`
//!
//! The installed service runs `axinite run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.axinite.daemon";
const SYSTEMD_UNIT: &str = "axinite.service";
...
"#,
    ...
    println!("  Start with: axinite service start");
    Ok(())
}
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `axinite service install` first.");
        }
...
fn axinite_logs_dir() -> PathBuf {
    axinite_base_dir().join("logs")
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.axinite.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/axinite.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_axinite() {
        let path = axinite_logs_dir();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".axinite/logs"), "unexpected path: {s}");
    }
}
