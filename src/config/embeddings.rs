...
mod tests {
    ...
    fn embeddings_disabled_not_overridden_by_openai_key() {
        let ctx = EnvContext::default().with_env("OPENAI_API_KEY", TEST_OPENAI_API_KEY_ISSUE_129);
        ...
        let config =
            EmbeddingsConfig::resolve_from(&ctx, &settings).expect("resolve should succeed");
        assert!(
            !config.enabled,
    ...
    fn embeddings_enabled_from_settings() {
        ...
        let config = EmbeddingsConfig::resolve_from(&EnvContext::default(), &settings)
    ...
    fn embeddings_env_override_takes_precedence() {
        let ctx = EnvContext::default().with_env("EMBEDDING_ENABLED", "true");
