...
/// Create the temporary refinery history table used by the repair tests.
///
/// Staging the table is arrangement, so the database error propagates to the
/// caller rather than panicking here.
#[cfg(feature = "postgres")]
pub(super) async fn create_temp_refinery_history_table(
    client: &Client,
...
/// Seed `rows` into the refinery history table, propagating database errors.
#[cfg(feature = "postgres")]
pub(super) async fn seed_history_rows<C: GenericClient>(
    client: &C,
    rows: &[(i32, &str, u64)],
) -> Result<(), tokio_postgres::Error> {
...
#[cfg(feature = "postgres")]
pub(super) async fn seed_legacy_released_rows<C: GenericClient>(
    client: &C,
) -> Result<(), tokio_postgres::Error> {
