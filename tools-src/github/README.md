...
3. Store the token:
   ...
   ```shell
   axinite secret set github_token YOUR_TOKEN
   ```
...
```text
GitHub token not found in secret store. Set it with: axinite secret set github_token <token>...
```
...
- Update the token: `axinite secret set github_token NEW_TOKEN`.
