...
- [ ] F1: RFC 0009 flag persistence (Rust): `feature_flag_overrides` table
...
- [ ] F4: chat media + auth cards + job cards (SPA): image staging
...
- [ ] F5: restart + TEE + pairing surfaces (SPA): restart button/modal/
...
- [ ] F6: jobs detail fidelity (SPA): Overview/Activity/Files tabs,
...
- [ ] F7: Python e2e migration: `?token=` boot + `data-testid` contract in
