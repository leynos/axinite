...
are all analysis. It keeps a paid label because it needs the Rust toolchain and
...
- Whitaker comes from the shared `install-whitaker` action, which downloads
  ...
  manifest and the release sidecar, and owns the installer cache.
- Nixie and Merman come from the shared `install-nixie` action, which installs
  pinned, checksum-verified releases of both.
...
one running `ci`, however identical its flags, so both coverage lanes now pass
`--profile ci` and the replacement is the whole of what it replaced. Without
that, standing `tests` down on a push would have left the compile contracts
...
`tests`, `telegram-tests`, `wasm-wit-compat` and `docker-build`, both
`coverage.yml` lanes and `codescene-coverage.yml` `coverage-check` are all
right-sized, each against a sampled peak and a warm wall time. The shapes and
the reasons are in the table under "Runner shape" above.

`docker-build` was previously excluded pending a Docker daemon and privilege
preflight. Shrinking a shape changes neither, and the job ran green at 605
seconds on `ubicloud-standard-4` with a 5,527 MiB peak, so the resize is
...
- Any job that gains a `services:` block, because a service container's
  networking is a separate preflight.
