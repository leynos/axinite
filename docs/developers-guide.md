...
#### Integration test support roots
...
Table: Test harness entrypoints, support roots, and ownership.
...
The methods `TraceLlm::from_file_async(...)`, `TraceLlm::calls()`, and
`TraceLlm::hint_mismatches()` are defined in
`tests/support/trace_provider_diagnostics.rs` in a separate `impl TraceLlm`
block; only support roots that include that file will compile those diagnostics.
...
forcing unrelated harnesses to compile it. Keep direct access to
`TraceLlmState::index`, `TraceLlm::inner`, and `TraceLlm::hint_mismatches`
inside sibling support modules that own trace-provider diagnostics; external
harness code should go through `TraceLlm::calls()` and
`TraceLlm::hint_mismatches()`.
...
Declared in `tests/support/trace_types.rs` and exposed only by support roots
that need trace fixtures, such as `tests/support/e2e.rs` and
`tests/support/support_unit.rs`.
