...
slice at a time, and each slice must produce its own cold-writer and warm-run
evidence before the next begins.

1. One low-risk Linux leaf job moves to `ubicloud-standard-2`, proving
...
5. `e2e.yml` `build` and `test` follow last among the build lanes.

Excluded from every slice until separately preflighted:

- `test.yml` `docker-build`, which needs a Docker daemon and privilege
  preflight.
