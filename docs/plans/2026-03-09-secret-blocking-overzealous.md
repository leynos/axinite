...
These tests should live as close to the wrappers and detector as possible. Favour deterministic unit or narrow integration tests over full worker orchestration.
...
- [x] 2026-03-13 18:03Z: Verified the PR gate failure against `origin/main` and bumped only the five registry manifest versions still matching base after extension-source edits: `registry/tools/github.json`, `registry/tools/slack.json`, `registry/channels/discord.json`, `registry/channels/slack.json`, and `registry/channels/whatsapp.json`. This pass deliberately leaves artefact URLs and checksums untouched because the failing gate checks only the manifest `version` fields and there is no current local validation requiring those URLs to change in lockstep.
- [x] 2026-03-13 15:33Z: Verified the latest review follow-ups against the live tree and fixed the still-open items only: removed the unused `mold` step from the WASM extension release job, rejected invalid extension kinds with HTTP 400 instead of silently coercing them to `None`, preserved auth-required activation details in the re-activate path, added typed prompt request parsing plus bounded job-event pagination with a `before_id` cursor, split `src/db/libsql/mod.rs` by moving shared helpers and row-conversion functions into dedicated submodules, and expanded the first `WASM` mention in this plan to `WebAssembly (WASM)`.
- [x] 2026-03-13 14:31Z: Verified the latest `vk pr` review threads against the live branch and fixed only the still-open items: removed the redundant `mold` install from the release job that only assembles global artefacts, reused the shared extension auth-response helper from the activate path, converted job-event responses to typed serializable DTOs, capped per-job prompt backlogs, replaced the remaining metadata-JSON test `.unwrap()` calls with contextual `expect(...)`, and tightened schema `properties` shape-mismatch merging with branch-order regression coverage.
...
- [x] 2026-03-13 13:18Z: Reduced duplicate and stale test coverage called out in review by consolidating schema-normalization, OAuth failure-path, rig-adapter helper, and truncation assertions, moving `truncate_for_preview` coverage back next to `agentic_loop`, and removing duplicate gateway util tests in favour of the dedicated chat-history helper suite.
...
Follow-up note, 2026-03-13 15:20Z: GitHub Actions was still using the invalid `taiki-e/install-action@cargo-component` shorthand even though `cargo-component` is not one of that action's tag-backed tool aliases. Replaced the five live occurrences across `test.yml`, `coverage.yml`, and `release.yml` with `taiki-e/install-action@v2` plus `with: { tool: cargo-component }`, then replayed YAML parsing checks and the standard gate stack before committing.

Follow-up note, 2026-03-13 15:35Z: `src/channels/web/handlers/extensions.rs` was still over the hard 400-line limit. Converted it into an `extensions/` module with `mod.rs` handling route composition only, a small shared helper module, and focused `listing`, `install`, `registry`, and `setup` handler files so the gateway keeps the same endpoint behaviour without breaching the file-size rule.
